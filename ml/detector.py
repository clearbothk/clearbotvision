...
class Detector:
    ...
    def __init__(self, model_path="model", parameter_path="camera_parameter", use_gpu=False, confidence_thres=0.5, nms_thres=0.3,
        ...
        self.weights_file = os.path.sep.join(
            [os.path.dirname(os.path.realpath(__file__)), model_path, weights_file])
        self.config_file = os.path.sep.join(
            [os.path.dirname(os.path.realpath(__file__)), model_path, config_file])
        # Ignoring camera calibration at this stage
        # self.matrix_file = os.path.sep.join(
        #     [os.path.dirname(os.path.realpath(__file__)), parameter_path, matrix_file])
        # self.distortion_file = os.path.sep.join(
        #     [os.path.dirname(os.path.realpath(__file__)), parameter_path, distortion_file])
        # self.rvecs_file = os.path.sep.join(
        #     [os.path.dirname(os.path.realpath(__file__)), parameter_path, rvecs_file])
        # self.tvecs_file = os.path.sep.join(
        #     [os.path.dirname(os.path.realpath(__file__)), parameter_path, tvecs_file])
        self.names_file = os.path.sep.join(
            [os.path.dirname(os.path.realpath(__file__)), model_path, names_file])
        logging.debug("Finished initialising model file paths")

        try:
            self.LABELS = open(self.names_file).read().strip().split("\n")
            logging.debug(
                f"Loaded labels from the names file: \n{self.LABELS}")
        ...
        # Camera calibration code. Will use later when further integration in the Pixhawk controller
        # try:
        ...
        unconnected_layers = functools.reduce(
            operator.iconcat, self.net.getUnconnectedOutLayers(), [])
